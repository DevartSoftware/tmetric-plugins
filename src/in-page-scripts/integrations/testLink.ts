    ...
    getIssue(_issueElement: HTMLElement, source: Source) {
        ...
        titleNodes.some((childNode, i) => {
            ...
            if (match) {
                ...
                if (!issueName && (i + 1) < titleNodes.length) { // description in a separate text node (ver 1.9.14)
        ...
        if (projectId && issueId) {
            issueUrl = `/linkto.php?tprojectPrefix=${projectId}&item=testcase&id=${issueId}`;
        }
